...
async def upload_image(request: Request, item_id: str, file: UploadFile = File(...)):
    ...
    rec = repository.add_image(item_id, StoredImageInput(stored.original_path, stored.thumb_path, stored.preview_path, width=stored.width, height=stored.height, file_sha256=stored.file_sha256))
    return rec
