...
export default function App() {
  ...
  return <div className={`app ${view === 'explore' ? 'explore-mode' : 'cards-mode'}`}>
    ...
    {standaloneGenerationOpen && <GenerationPanel t={t} preferredLanguage={preferredLanguage} tags={tags} initialJobId={focusedGenerationJobId} onClose={() => setStandaloneGenerationOpen(false)} onAccepted={(item, message) => { saved(); setToast({ title: message || 'New variant item created', tone: 'success' }); if (item?.id) setDetailId(item.id); }} />}
  </div>
}
