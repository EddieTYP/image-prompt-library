...
Current status: **Batch 1 ImportDraft core**, **Batch 2 local markdown repository ingestion**, **Batch 3 GenerationJob plus result inbox foundation**, and **Batch 4 `openai_codex_oauth_native` backend/provider UI slices** are implemented. Generation jobs can be created provider-agnostically, receive manually staged or native Codex-provider staged result images under `generation-results/`, be listed/reviewed, and accept/discard results; accepted results are copied into normal library media storage. The Config drawer now lists optional generation providers and exposes the native Codex connect/poll/disconnect controls for local installs. Next implementation milestone: **Batch 4.4 Generation UX** and then **Batch 5 generic URL plus X/Threads import**.
...
- Build Generation UX/result inbox controls (launch generation from a saved prompt, run provider jobs, review generated results, accept/retry/discard) on top of the backend endpoints.
