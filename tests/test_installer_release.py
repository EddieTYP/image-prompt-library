...
def test_readme_prefers_installer_for_users_and_keeps_source_setup_for_developers():
    ...
    assert "image-prompt-library update --version v0.7.0-beta" in installation
    assert "curl -fsSL https://raw.githubusercontent.com/EddieTYP/image-prompt-library/main/scripts/install.sh | bash -s -- --version v0.7.0-beta" in installation
