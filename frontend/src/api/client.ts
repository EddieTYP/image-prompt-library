...
export const api = isDemoMode ? {
  ...
  generationProviders: () => Promise.resolve<GenerationProviderStatus[]>([
    ...
    },
    {
      ...
      state: 'demo_unavailable',
      status: 'unavailable',
      message: 'Generation requires a local install and XAI_API_KEY.',
      can_generate: false,
      reason: 'local_only',
      features: { text_to_image: false, text_reference_to_image: false, image_edit: false },
  ...
  xaiAPIKeySave: (_apiKey: string) => demoReadOnly(),
  xaiAPIKeyDelete: () => demoReadOnly(),
  ...
  xaiAPIKeySave: (apiKey: string) => json<GenerationProviderStatus>('/api/generation-providers/xai-api/api-key', { method: 'POST', body: JSON.stringify({ api_key: apiKey }) }),
  xaiAPIKeyDelete: () => json<GenerationProviderStatus>('/api/generation-providers/xai-api/api-key', { method: 'DELETE' }),
