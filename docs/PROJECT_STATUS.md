...
4. **`openai_codex_oauth_native` provider — first backend slice done**: app-owned native Codex auth store outside the library, redacted provider status API, device-code start/poll helpers, Codex-compatible headers, and `POST /api/generation-jobs/{job_id}/run` can stage Codex image results into the GenerationJob inbox. Frontend UI, live-account QA, refresh/lock hardening, reference/edit modes, and retry controls remain follow-ups.
...
- Batch 4 `openai_codex_oauth_native` first backend slice is now implemented: `backend/services/openai_codex_native.py` owns the app-native auth store (`~/.image-prompt-library/auth.json` by default, overrideable via `IMAGE_PROMPT_LIBRARY_AUTH_PATH`), redacted token status, device-code start/poll helpers, `ChatGPT-Account-ID` JWT header extraction, Codex-compatible headers, Codex Responses `image_generation` streaming, and result staging through the existing GenerationJob inbox. `backend/routers/generation_providers.py` exposes `GET /api/generation-providers/openai-codex-native/status`, `POST /api/generation-providers/openai-codex-native/auth/start`, and `POST /api/generation-providers/openai-codex-native/auth/poll`; `POST /api/generation-jobs/{job_id}/run` executes queued jobs whose provider is `openai_codex_oauth_native`. Starting native auth currently requires `IMAGE_PROMPT_LIBRARY_CODEX_CLIENT_ID`; frontend UI, live-account QA, token refresh/lock hardening, Text+Reference/Image Edit payloads, and retry controls remain follow-ups.
