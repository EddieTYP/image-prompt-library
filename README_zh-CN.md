...
当前公开 beta：[`v0.6.5-beta`](https://github.com/EddieTYP/image-prompt-library/releases/tag/v0.6.5-beta)。这个版本更新了 `gpt_image_2_skill` 和 `awesome-gpt-image-2` 的公开 sample metadata，修正双语 prompt variants，加入 demo 用的本地化 card titles，并保留 v0.6 beta 系列的本地生成、installer、update 和 rollback workflow。
