...
4. **`openai_codex_oauth_native` provider — backend slice plus optional-provider hardening done**: app-owned native Codex auth store outside the library, frontend-ready optional provider status/list API, device-code start/poll/disconnect helpers, env/local-config client-id bootstrap, token refresh before expiry, Codex-compatible headers, and `POST /api/generation-jobs/{job_id}/run` can stage Codex image results into the GenerationJob inbox. Frontend UI, live-account QA, refresh lock hardening, reference/edit modes, and retry controls remain follow-ups.
